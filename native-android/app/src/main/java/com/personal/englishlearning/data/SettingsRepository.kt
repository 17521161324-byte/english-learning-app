...
data class LearningSettings(
    ...
    val dailySpeakingSessions: Int = 1,
...
class SettingsRepository(private val context: Context) {
    private object Keys {
        ...
        val dailySpeakingSessions = intPreferencesKey("daily_speaking_sessions")
    ...
    val settings: Flow<LearningSettings> = context.settingsDataStore.data.map { values ->
        LearningSettings(
            ...
            dailySpeakingSessions = values[Keys.dailySpeakingSessions] ?: 1,
            dailyMinutes = values[Keys.dailyMinutes] ?: 30,
            remindersEnabled = values[Keys.remindersEnabled] ?: false,
        )
    }

    suspend fun updateGoals(newWords: Int, reviewWords: Int, speakingSessions: Int, minutes: Int) {
        context.settingsDataStore.edit { values ->
            ...
            values[Keys.dailySpeakingSessions] = speakingSessions.coerceIn(0, 20)
